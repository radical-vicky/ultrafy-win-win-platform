...
import { notifyAdminNewUserPending } from "@/lib/mailer";
import { rateLimit, getClientIp } from "@/lib/rate-limit";

export async function POST(req: NextRequest) {
  const limited = await rateLimit(`verify-email:${getClientIp(req)}`, 10, 15 * 60 * 1000);
  if (!limited.success) {
  ...
  if (user.emailVerified) {
    return NextResponse.json({ error: "This email is already verified — try logging in." }, { status: 400 });
  }

  const otp = await prisma.emailOtp.findFirst({
    where: { userId: user.id, usedAt: null, purpose: "SIGNUP" },
  ...
  notifyAdminNewUserPending({
    name: user.name,
    email: user.email,
    role: user.role,
    reviewUrl: `${process.env.NEXT_PUBLIC_APP_URL}/admin/users`,
  }).catch((err) => console.error("[mailer] admin pending-user notify failed", err));

  // Email verified — log them in now. Full access (e.g. listing a property)
  // still requires admin approval, enforced where those actions happen.
